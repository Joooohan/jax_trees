...
from jax_trees.jax.classifier import (compute_all_scores,
...
def test_compute_column_scores():
    ...
    assert jnp.allclose(
        scores,
        np.asarray([0.0, 2 / 3, np.nan, np.nan]),
        equal_nan=True,
        rtol=1e-1,
    )
...
def test_compute_all_scores():
    ...
    assert jnp.allclose(
        scores,
        np.stack([[0.0, np.nan], [np.nan, 1.0]], axis=1),
        equal_nan=True,
    )
